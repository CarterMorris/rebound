...
EM_JS(void, init_keyboard_capture, (), {
    // When a key is pressed, send it to the main window
    window.onkeydown = (e) => {
        port = window.port;

        if (port){
            port.postMessage({ type: "keyboard", data: e.key });
        };
    };
});

EM_JS(void, reb_link_message_channel, (), {
    console.log("Running:reb_link_message_channel");
    // Receive the message channel port from window.onmessage
    window.onmessage = (e) => {
        console.log("Received window message", e);
        if (e.ports && e.ports[0]) {
            console.log("Received message channel port");

            var port = e.ports[0];
            port.onmessage = port_onmessage;
            window.port = port;         // Attach port to global window for later access

            // Communicate to main window that port has been received
            port.postMessage({ type: "init", data: "" })
        };
    };
    
    // Handler for incoming messages
    function port_onmessage (e){
        console.log(e.data);
    };
